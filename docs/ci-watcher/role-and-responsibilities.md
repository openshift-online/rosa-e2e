...
| Step | Task | Tool |
|------|------|------|
| 1 | Review the automated [daily status report](../../scripts/ci-status-report.sh) posted to `#wg-rosa-ci-enhancement` | Slack |
| 2 | Run `/ci-triage` to get automated status, failure diagnosis, and proposed actions | Claude Code |
...
| 4 | For any failures the AI could not classify, investigate root cause manually | Prow GCS |
| 5 | Review and approve any Jira stories or fix PRs proposed by `/ci-triage` | Jira / GitHub |
| 6 | File or update Jira for anything `/ci-triage` missed | Jira ([ROSAENG-391](https://redhat.atlassian.net/browse/ROSAENG-391)) |
...
| Day | Task |
|-----|------|
| Monday | A Slack workflow will remind the upcoming ci-watcher to review the handover doc, current status at UTC 0:00 |
| Mon-Thu | Daily triage: run `/ci-triage`, review findings, approve/correct, post status. Shepherd open PRs |
| Friday | A Slack workflow will remind the current ci-watcher to fill the handover doc at UTC 6:00 |
