...
1. **Check for boilerplate directory**: Use GitHub tools to verify the repo has a `boilerplate/` directory. If not, skip silently.
...
6. **Push the branch**: Push to the bot's fork of the operator repo.

7. **Create a PR**: Create a pull request on the operator's GitHub repo:
...
- Some repos may have an existing open boilerplate update PR. Before creating a new PR, check if one already exists (search for open PRs with `boilerplate-update` in the branch name). If one exists, skip the operator and note it in the thread:
  > ⏭️ **<operator-name>** — Existing boilerplate update PR already open: <PR link>
