...
6. **Push the branch**: Push to the `fork` remote (the bot's fork). If the push fails, post a threaded failure reply, increment the Failed count, and skip to the next operator — do not create the PR.
